'use strict';
/**
 * EXP-050 — a volatility-ratio-adjusted stop on IDX. Stages 0-2 only.
 *
 * Pre-registered in PREREGISTRATION_2026-09-02_idx_volratio_stop.md.
 *
 * ── THE AWKWARD PART, STATED FIRST ───────────────────────────────────────────
 *
 * If recent volatility sits above its own longer-run level it tends to fall, so a
 * stop at 2.5 x ATR is wider than the coming 20 sessions warrant and should be
 * hit less often. That is close to arithmetic, not insight. A Stage 0 pass here
 * shows only that the deployed rule ignores a forty-year-old fact.
 *
...
(async () => {
  ...
  console.log('EXP-050 — a volatility-ratio-adjusted stop on IDX. Stages 0-2 only.');
  console.log('  pre-registered in PREREGISTRATION_2026-09-02_idx_volratio_stop.md — TWO-SIDED');
  console.log('  STATED FIRST: the Stage 0 prediction is near-mechanical. A pass shows only that');
  console.log('  the deployed rule ignores a known fact. STAGE 2 is the interesting part.');
  ...
  const icSeries = anchors.map(d => {
    const rows = byDate.get(d);
    return spearman(rows.map(r => r.volRatio), rows.map(r => r.range));
  }).filter(v => v !== null && Number.isFinite(v));
  const c1 = oneSample(icSeries);
  const c1ok = c1.mean >= CONTROL_IC_BAND[0] && c1.mean <= CONTROL_IC_BAND[1] && c1.p < CONTROL_MAX_P;
  console.log(`  1. VOL_RATIO vs 20-session range: IC ${f4(c1.mean)}  t ${c1.t.toFixed(2)}   ` +
    `must land in [${CONTROL_IC_BAND[0]}, ${CONTROL_IC_BAND[1]}] to match EXP-049b's -0.1698 ... ${c1ok ? 'PASS' : 'FAIL'}`);
