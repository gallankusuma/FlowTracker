'use strict';
/**
 * EXP-037 — the single pre-registered hypothesis from
 * PREREGISTRATION_2026-08-30_zone_profitability.md.
 *
 * H1: buying when the close enters a SUPPORT zone and holding 20 sessions
 *     returns more, NET OF COSTS, than the same stock returned unconditionally
 *     over the same horizon in the same period. One-sided.
 *
 * ── THIS IS NOT IMPLIED BY EXP-036 ───────────────────────────────────────────
 *
 * EXP-036 found pivots land in zones ~1.4pp more often once proximity is held
 * constant. A pivot is a turn in EITHER direction: a zone that catches more
 * pivots catches more tops as well as more bottoms. And a pivot is only
 * identifiable three bars later, by which time the price that defined it is
 * gone -- so "buy the pivot" is a rule using information it never had.
 *
 * ── THE THREE WAYS THIS COULD LIE, AND WHAT IS DONE ABOUT EACH ───────────────
 *
 * 1. FILLING AT THE SIGNAL CLOSE. The close that triggers is only known after
 *    the session ends. Entry is the NEXT OPEN.
 * 2. BEATING ZERO INSTEAD OF THE STOCK. IDX names drift; +0.3% over 20 sessions
 *    is nothing if the stock returned +0.3% over every 20 sessions. The
 *    benchmark is the ticker's own unconditional 20-session mean.
 * 3. IGNORING COSTS. 0.6% round trip, fixed before the run. Most level effects
 *    are smaller than this, and that is the finding when it happens.
 *
 * No stop is simulated: daily bars do not record whether the low came before the
 * high, so a stop would be inventing intrabar path. This measures a
 * hold-through-anything rule, stated rather than hidden.
 *
 * *** SURVIVORSHIP-BIASED RESEARCH RESULT ***
 * backfill_price_history.js only ever fetched tickers present in
 * idx_broker_summary, and twelve suspended/delisted names were removed in July
 * 2026 -- precisely the ones that would have blown up. This result is biased
 * upward and the banner stays regardless of outcome.
 *
 * Usage: node scraper/research/exp037_zone_profitability.js
 */
...
(async () => {
  ...
  console.log('EXP-037 — pre-registered: is buying a volume support zone profitable?');
  console.log('  one-sided, direction fixed in PREREGISTRATION_2026-08-30_zone_profitability.md');
  console.log(`  entry at the NEXT OPEN after the close enters a support zone; hold ${HOLD} sessions`);
  ...
  for (const u of uni) {
    ...
    for (let t = PRIOR; t + HOLD + 1 < bars.length; t++) {
      ...
      // A SUPPORT zone: one price was above LOOKBACK_ABOVE sessions ago, that the
      // close has just fallen into having been above it the session before.
      const hit = zoneSet.find(z =>
        z.hi < refClose && close >= z.lo && close <= z.hi && prevClose > z.hi);
      if (!hit) continue;
  ...
  console.log(`  one-sided p      : ${p.toFixed(5)}`);
  const profitable = p < 0.05 && m > 0;
  console.log(`  VERDICT          : ${profitable ? 'PROFITABLE' : 'NOT PROFITABLE'} against the pre-registered rule`);
