...
async function refreshUSStockPrices() {
  ...
  for (const ticker of US_TICKERS) {
    try {
      // roundPrices:false is mandatory here — US equities are quoted in cents.
      // See the fetcher's header for what the default silently did to this table.
      const { candles } = await fetchYahooCandles(ticker, '5d', '', { roundPrices: false });
...
// The US composite moved to modules/us_score_engine.js so the signal-history
// backfill can score through the SAME function this scanner does. A backfilled
// history that scores differently from the live scanner measures a system
// nobody is running -- and re-implementing the assembly is the exact drift
// defect modules/score_engine.js was created to stop on the IDX side.
//
// Thresholds stay live here: the core takes them injected, and passing
// getActiveThresholds() keeps this path byte-identical to the inline version it
// replaces (which called server.js classifySignal, i.e. the optimized
// thresholds). The backfill passes DEFAULT_THRESHOLDS instead and records that.
const {
  computeUSStockFactors: computeUSStockFactorsCore,
  US_TECH_WEIGHTS,
} = require('./modules/us_score_engine');

function computeUSStockFactors(candles, marketDirection, marketAvgChangePct = 0) {
  return computeUSStockFactorsCore(candles, marketDirection, marketAvgChangePct, {
